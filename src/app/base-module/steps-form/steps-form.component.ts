...
export class StepsFormComponent implements OnInit, AfterViewInit {
  ...
  generalInfoDraftLoader: boolean;
  furtherInfoDraftLoader: boolean;
  postCodeType: string = "number";
  showPostCodeError: boolean;
  generalInfoDraft: boolean;
  furtherInfoDraft: boolean;
  draftLength: number;
  storeAll: any[] = [];
  gardenSurface: string;
  draft: boolean;
  furtherDraft: boolean;
  ...
  ngOnInit(): void {
    ...
    this.surafceSQFT = "m2";
    // console.log('specific contact', this.specificContact);
    this.sharedservice.getMockList().subscribe((Response) => {
      ...
      if (this.heatingTypeList) {
        for (let i = 0; i < this.heatingTypeList.length; i++) {
          ...
          this.translate
            .get("label." + `${this.heatingTypeSingle}`)
            .subscribe((data: any) => {
      ...
      if (this.kitchenTypeList) {
        for (let i = 0; i < this.kitchenTypeList.length; i++) {
          ...
          this.translate
            .get("label." + `${this.kitchenTypeSingle}`)
            .subscribe((data: any) => {
    ...
    // this.mobNumberPattern = "^(?:(?:\+|0{0,2})91(\s*[\-]\s*)?|[0]?)?[789]\d{9}$";
    // new google.maps.Map(document.getElementById('map') as HTMLElement,{
    //   center:{lat:-33.8686,lng:151.2195},
    //   zoom:13
    // })
  }
  mapInitializer() {
  ...
  updateObjValue2(event) {
    if (this.countryCode.length === 3) {
      this.styleObject = { ["width"]: "50px" };
    }
    if (this.countryCode.length === 4) {
      this.styleObject = { ["width"]: "60px" };
    }
    if (this.countryCode.length === 6) {
      this.styleObject = { ["width"]: "72px" };
    }
  }
  ...
  click(event: google.maps.MouseEvent): void {
    ...
    this.resultCenter = {
      lat: this.markersData[0].position.lat,
      lng: this.markersData[0].position.lng,
    };
    this.resultsZoom = 7;
  }
  ...
  navigateToFurther(): void {
    window.scrollTo(0, 0);
    if (
      this.listingRefernce === undefined ||
      this.listingRefernce === null ||
      this.listingRefernce === ""
    ) {
      const chars =
      ...
      this.listingRefernce = randomstring.replace(
        /[^A-Z0-9]/g,
        function (match) {
          if (match != undefined) {
    ...
    // console.log(this.info.furnished_property)
    if (
      this.listingTitle === "" ||
      this.homeType === "" ||
      ...
      this.furnishedProperty == undefined ||
      ((this.landSurface === "" ||
        this.landSurface === undefined ||
        this.landSurface === null) &&
        (this.homeType === "house" ||
          this.homeType === "hotel" ||
          this.homeType === "villa" ||
          this.homeType === "mansion")) ||
      this.getExcludingFee() === undefined ||
      this.getExcludingFee() === null ||
      this.getExcludingFee() === 0 ||
      this.includingFees === undefined ||
      this.includingFees === null ||
      this.endFees === undefined ||
      this.endFees === null ||
      this.getATIval() === NaN ||
      this.getATIval() === undefined ||
      this.getATIval() === null ||
      this.getPricepesqm() === "NaN" ||
      this.getPricepesqm() === "Infinity" ||
      this.getPricepesqm() === "0.00" ||
      ...
      for (let i = 0; i < this.storeAll.length; i++) {
        if (
          this.storeAll[i].value == undefined ||
          this.storeAll[i].value == null ||
          this.storeAll[i].value == ""
        ) {
          value = this.el.nativeElement.querySelector(
            "#" + this.storeAll[i].id
          );
          break;
        }
      ...
      let len = this.translate.instant("label.draft").length;
      len > 20 ? (this.draftLength = len * 9) : (this.draftLength = len * 12);
      // this.generalInfoDraftLoader = true;
      this.generalInfoDraft
        ? (this.generalInfoDraftLoader = true)
        : (this.generalInfoLoader = true);
      this.sharedservice.getListingGeneralInfo(this.info).subscribe(
        (Response) => {
          this.generalInfoDraft
            ? (this.generalInfoDraftLoader = false)
          ...
          if (this.generalResponse.status === 1) {
            ...
            sessionStorage.setItem(
              "createListing-generalInfoId",
              this.generalResponse.listing_id
            );
            if (this.generalInfoDraft == true) {
              this.toastr.success(
                this.translate.instant("label.draft_notify"),
                "Success",
                {closeButton: true}
              );
              this.generalInfoDraft = false;
              this.tabName = "general";
            } else {
              this.toastr.success(this.generalResponse.message, "Sucess", {closeButton: true});
              this.tabName = "further";
            }
          } else {
            if (
              this.generalResponse.status === 0 &&
              this.generalResponse.errors
            ) {
              if (this.generalResponse.errors.num_rooms) {
                this.toastr.error(
                  this.generalResponse.errors.num_rooms[0],
                  "Error",
                  {closeButton: true}
                );
              } else if (this.generalResponse.errors.land_price) {
                this.toastr.error(
                  this.generalResponse.errors.land_price[0],
                  "Error",
                  {closeButton: true}
                );
              } else {
                this.toastr.error(this.generalResponse.message, "Error", {closeButton: true});
                // this.generalInfoLoader = true;
              }
            } else {
              this.toastr.error(this.generalResponse.message, "Error", {closeButton: true});
        ...
        (error) => {
          this.toastr.error("The given data was invalid", "Error", {closeButton: true});
          this.generalInfoLoader = false;
        }
      );
    }
  ...
  navigateToValidation(): void {
    this.contactDetails = {
      email: this.pstOfc ? this.pstOfc : "",
      mobile: this.cellNo ? this.cellNo : "",
    };
    ...
    const furtherInfo = {
      ...
      apartment_floor:
        this.selectedHomeType === "apartment" ? this.apartmentFloors : "",
      ...
      draft: this.furtherDraft ? 1 : 0,
    };
    if (
      this.noOfBedrooms === undefined ||
      this.noOfBedrooms === null ||
      this.noOfBedrooms === "" ||
      this.addedPhotos.length === 0
    ) {
      ...
      for (let i = 0; i < this.storeFurtherAll.length; i++) {
        if (
          this.storeFurtherAll[i].value == undefined ||
          this.storeFurtherAll[i].value == null ||
          this.storeFurtherAll[i].value == ""
        ) {
          value = this.el.nativeElement.querySelector(
            "#" + this.storeFurtherAll[i].id
          );
          break;
        }
      }
      ...
      this.furtherInfoDraft
        ? (this.furtherInfoDraftLoader = true)
        : (this.furtherInfoLoader = true);
      this.sharedservice.getListingFurtherInfo(furtherInfo).subscribe(
        (response) => {
          this.furtherInfoDraft
            ? (this.furtherInfoDraftLoader = false)
            : (this.furtherInfoLoader = false);

          this.furtherResponse = response;
          if (this.furtherResponse.status === 1) {
            if (this.furtherInfoDraft == true) {
              this.toastr.success(
                this.translate.instant("label.draft_notify"),
                "Success",
                {
  ...
  //uploading thumbnails of videos
  ...
  checkCheckBoxvalue(event, directions): void {
    this.orientation = _.map(
      _.filter(this.orientationDirections, { value: true }),
      "name"
    );
  }
  ...
  publishAdd(): void {
    ...
    this.sharedservice
      .getListingPublishedInfo({ listing_id: this.listingID })
      .subscribe(
        (Response) => {
          ...
          if (this.publishAddResponse.status === 1) {
            this.toastr.success(this.publishAddResponse.message, "Success", {closeButton: true});
            this.router.navigate([""]);
          } else {
  ...
  getATIval() {
    if(this.excludingFees && this.includingFees){
    ...
    if (
      ...
      return (
        (this.includingFees / this.excludingFees) * 100 -
        100
      ).toFixed(2);
    } else if (this.endFees === "seller") {
      this.feesATI = this.feesATI ? this.feesATI : 0;
      return this.feesATI;
    }
  ...
  getPricepesqm() {
    if(this.includingFees && this.surface){
  ...
  getSelectedPerson(): void {
    ...
    this.translate
      .get("label." + `${this.selectedEndFeesForValidDisplay}`)
      .subscribe((data: any) => {
        this.selectedEndFeesForValidDisplay = data;
      });
  }
  ...
  rotateImage(events): void {
    ...
    const j = i + 1;
    const value =
      document.getElementById("custom-ndip").children[j].children[0];
  ...
  getSelectedHomeType(homeType): void {
    ...
    this.rmVal();
    this.selectedHomeTypeForValidDisplay = this.toTitleCase(
      this.selectedHomeType
    );
    this.translate
      .get("label." + `${this.selectedHomeTypeForValidDisplay}`)
      .subscribe((data: any) => {
  ...
  selectKitchen(kitchen): void {
    ...
    this.translate
      .get("label." + `${this.selectedKitchenForValidDisplay}`)
      .subscribe((data: any) => {
  ...
  selectedHeat(heat): void {
    ...
    this.translate
      .get("label." + `${this.selectedHeatForValidDisplay}`)
      .subscribe((data: any) => {
  ...
  findLocation(postalCode, commune, address, quarter, pays): void {
    ...
    location.place.geometry?.viewport
      ? this.map.fitBounds(location.place.geometry.viewport)
      : this.map.setCenter(location.place.geometry.location);
  }
  adfd(d) {
    return (this.showPostCodeError = d ? false : true);
  }
  //exact location with marker
  getExactLocation() {
    if (this.address) {
      const myLatLng = {
        lat: this.exactLocationLatitude,
        lng: this.exactLocationLangitude,
      };
  ...
  tabChanged() {
    for (let i = 0; i < this.addedPhotos.length; i++) {
      ...
      if (this.tabName == "further") {
        ...
        value = document.getElementById("custom-ndip-preview").children[i]
          .children[0];
      }
      this.renderer.setStyle(
        value,
        "transform",
        `rotate(${this.addedPhotos[i]["angle"]}deg)`
      );
    }
  }
  ...
  generate_thumbnails(url, quality) {
    if (url) {
      ...
      if ((result = url.match(/youtube\.com.*(\?v=|\/embed\/)(.{11})/))) {
        ...
        (result = url.match(/player.previsite\.net\/mooveo\/[A-Z0-9]/))
      ) {
  ...
  storeThumbnail(url, thumbnail) {
    ...
    this.noDuplicateVideos = Array.from(
      new Set(this.addedVideos.map((a) => a.videoUrl))
    ).map((id) => {
      return this.addedVideos.find((f) => f.videoUrl === id);
    });
    this.uniqueVideos = Array.from(
      new Set(this.noDuplicateVideos.map((m) => m.videoUrl))
    );
  }
  //youtube thumbnail
  getYoutubeThumbnail(id, quality): string {
    ...
    if (id) {
      ...
      return (thumbnail =
        "http://img.youtube.com/vi/" + id + "/" + quality_key + ".jpg");
    }
  }
  ...
  getDailyMotionThumbnail(str): string {
    ...
    return (videoThumbnail =
      "https://www.dailymotion.com/thumbnail/video/" + ret[0]);
  }
  ...
  //save general info draft
  saveGeneralInfoDraft() {
    this.draft = true;
    this.generalInfoDraft = true;
    this.navigateToFurther();
    this.draft = false;
  }
  //save further info draft
  saveFurtherInfoDraft() {
    this.furtherDraft = true;
    this.furtherInfoDraft = true;
    this.navigateToValidation();
    this.furtherDraft = false;
  }
